...
class SubtitleOptimizer:
    ...
    def _parallel_optimize(self, chunks: List[Dict[str, str]]) -> Dict[str, str]:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # 对于失败的块，保留原文
                pending_results[index] = {k: v for k, v in chunk.items()}
    ...
    def _safe_optimize_chunk(
        self, chunk: Dict[str, str], context_before: str = ""
    ) -> Dict[str, str]:
        """安全的优化块，包含重试逻辑"""
        for i in range(self.retry_times):
            try:
                return self._optimize_chunk(chunk, context_before=context_before)
            except Exception as e:
                if i == self.retry_times - 1:
                    raise
                logger.warning(
                    f"优化重试 {i+1}/{self.retry_times}: {format_openai_compat_error(e, model_name=self.model)}"
                )
        return chunk
    ...
    def _optimize_chunk(
        ...
        if context_before:
            user_prompt += (
                ...
                "Do not rewrite or output this context directly."
        ...
        if self.usage_callback:
            self.usage_callback("optimize", extract_openai_usage(response))
